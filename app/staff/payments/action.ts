...
export async function readAllPayments(
  ...
  try {
    const permission = await hasPermission(PERMISSION_MODULES.PAYMENTS, PERMISSION_ACTIONS.READ)
    if (!permission) {
      return { success: false, error: "You don't have permission to view payments" }
    }
