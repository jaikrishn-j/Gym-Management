...
import { plans, memberplans, payments, pendingSyncs, planRequests } from "@/drizzle/schema"
...
import { memberAttendance } from "@/drizzle/schema"
    ...
    for (const record of pending) {
      try {
        ...
        if (record.action === "create_payment") {
          ...
          const perm = await hasPermission(PERMISSION_MODULES.MEMBERS, PERMISSION_ACTIONS.UPDATE)
          if (perm) {
            const existing = await db
              ...
              .where(
                and(
                  eq(memberAttendance.clerkUserId, payload.clerkUserId),
                  sql`DATE(${memberAttendance.date}) = DATE(${new Date(payload.date).toISOString()})`,
                )
              )
              .limit(1)
            ...
            if (existing.length > 0) {
              await db
                ...
                .set({
                  timeIn: payload.timeIn ? new Date(payload.timeIn) : undefined,
                  timeOut: payload.timeOut ? new Date(payload.timeOut) : undefined,
                  weightIn: payload.weightIn ?? undefined,
                  weightOut: payload.weightOut ?? undefined,
              ...
              await db.insert(memberAttendance).values({
                ...
                timeIn: payload.timeIn ? new Date(payload.timeIn) : null,
                timeOut: payload.timeOut ? new Date(payload.timeOut) : null,
                weightIn: payload.weightIn ?? null,
                weightOut: payload.weightOut ?? null,
                isPresent: payload.isPresent ?? true,
              })
            }
...
export async function approvePlanRequest(
  ...
  try {
    ...
    // Check if user has an active plan already — extend it
