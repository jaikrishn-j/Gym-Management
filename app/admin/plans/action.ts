...
// Helper function to map DB record to Plan interface (temporary fix for typo)
...
export async function getPlanById(id: string) {
    try {
        ...
        return { success: true, data: mapDbToPlan(plan) }
    } catch (error) {
...
// TOGGLE - Toggle plan active status
export async function togglePlanStatus(id: string) {
    try {
        // Check permission (uses UPDATE permission since it's modifying)
        const permission = await hasPermission(PERMISSION_MODULES.PLANS, PERMISSION_ACTIONS.UPDATE)
        if (!permission) {
            return { success: false, error: "You don't have permission to modify plans" }
        }
